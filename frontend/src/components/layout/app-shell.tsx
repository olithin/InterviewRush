...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const problemDetailWide =
    ...
    /^\/interview\/(csharp|java|python|oop)\/[^/]+$/.test(pathname ?? "") ||
  ...
  return (
    <div className="min-h-screen px-4 py-6 md:px-8">
      ...
      >
        <header className="clay-surface relative z-30 rounded-[2.4rem_2rem_2.3rem_2.05rem/2.05rem_2.2rem_1.9rem_2.15rem] border border-white/65 bg-gradient-to-b from-[hsl(48,48%,99.2%)] via-[hsl(42,38%,98%)] to-[hsl(35,30%,90%)] p-4 shadow-clay ring-1 ring-inset ring-amber-50/50">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0 flex-1 pr-2">
              ...
              {!isLogin ? (
                <p className="mt-0.5 max-w-md text-xs font-medium leading-relaxed text-muted-foreground">
                  Interview prep for C#, Java, and Python — one small quest at a time.
                </p>
              ) : (
                <p className="mt-0.5 max-w-md text-xs font-medium leading-relaxed text-muted-foreground">
            ...
            {!isLogin ? (
            <nav className="flex flex-1 flex-wrap justify-end gap-1.5 md:max-w-[min(100%,42rem)]">
              {navItems.map((item) => {
                const Icon = iconMap[item.label as keyof typeof iconMap] ?? Trophy;
                const active =
                  pathname === item.href ||
                  (item.href === "/problems" && (pathname?.startsWith("/problems/") ?? false)) ||
                  (item.href === "/interview" && (pathname?.startsWith("/interview") ?? false)) ||
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    className={cn(
                      "relative inline-flex min-h-8 min-w-8 items-center gap-1.5 rounded-full px-2.5 py-1.5 text-xs font-semibold transition",
                      active
                        ? "text-primary"
